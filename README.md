...
3.4.0 版沿用轻量安装结构，不包含大型模型或数百 MB 的原始音频。
...
浏览器扩展版适合所有工作都在浏览器中完成的用户，不需要运行 Windows EXE。3.4.0 起，它不再只是一个小弹窗：扩展会打开与网页版同一套完整专注台，同时由后台持续监督当前标签页。
...
```powershell
.\scripts\build_public_editions.ps1 -Version 3.4.0
```
